...
import com.haiqing.conference.util.JsonUtil;
...
public class PersonnelController {
     ...
     * @return
     */
    @PostMapping("/personnelsIds")
    public ResultBean queryPersonnelsIds(@RequestBody String ids) {
        log.info("- enter into method PersonnelController.queryPersonnelsIds,parameter ids:{}", (Object) ids);
        if (EmptyUtility.strIsEmpty(ids)) {
            return new ResultBean(1045);
        }
        HashMap idss = JsonUtil.toObject(ids, HashMap.class);
        System.out.println(idss);
        System.out.println(idss.get("ids"));
        return personnelService.getPersonnersByIds(JsonUtil.toObject(idss.get("ids").toString(), Integer[].class));
    }
    ...
    public ResultBean putPersonnel(@RequestBody Personnel personnel) {
        log.info("- enter into method PersonnelController.putPersonnel,parameter, personnel:{}", personnel);
        //加入更新数据
